      ...
      @Override protected void onPostExecute(DemoModeAvailability demoModeAvailability) {
        switch (demoModeAvailability) {
          ...
          case NEEDS_ROOT_ACCESS:
            callback.hide();
            Toast.makeText(activity, com.jakewharton.screenrecord.R.string.root_permission_denied, LENGTH_SHORT).show();
            break;
          case NEEDS_DEMO_MODE_SETTING:
            callback.hide();
            Toast.makeText(activity, com.jakewharton.screenrecord.R.string.enable_demo_mode_in_settings, LENGTH_SHORT).show();
